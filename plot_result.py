...
def plot_result(Samp, X, c, n, outfile, Ngrid=100, M=4):
    ...
    #                 print(samples.M)
    #                 print(D)
    #                 s_l = np.reshape(samples.s_l, (samples.M, D*D))
    #                 s_r = np.reshape(samples.s_r, (samples.M, D*D))
    #                 m = np.reshape(samples.mu, (samples.M, D))
    #                 p = np.reshape(np.array(np.squeeze(samples.pi)), (-1, 1))
    #
    #                 # loop over components in this sample
    #                 for b in range(samples.M):
    #                     tempC = inv(np.reshape(s_l[b, :], (D, D)))
